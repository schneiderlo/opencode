...
Configure the Planner → Executor → Synthesizer workflow via the `heavy` key.
...
```jsonc title="opencode.jsonc"
{
  ...
  "heavy": {
    ...
    // Execution control
    "max_concurrent_agents": 3,            // Positive integer concurrency limit
    "strategy": "parallel",               // "parallel" | "sequential" | "tree"

    // Optional agent pool (round-robin per task)
    "agent_pool_models": [
      "openai/gpt-4o-mini",
      "anthropic/claude-3-haiku"
    ],

    // Tool access controls (optional)
    "tools": {
      "read_only": true,
    ...
    // Retry logic for sub-tasks (optional)
    "retry": {
      "max_attempts": 3,
      "backoff_ms": 1000,
      "rotate_models": true,
...
- If `planner_model`/`synthesizer_model` are not set, the session's model is used.
- When `read_only` is true, potentially destructive tools are disabled for sub-agents.
- `agent_pool_models` enables round-robin model selection per sub-task.
- Concurrency is bounded by `max_concurrent_agents` and the number of sub-tasks.
